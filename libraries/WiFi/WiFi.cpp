...
volatile bool WiFiClass::_connecting = false;
volatile bool WiFiClass::DISCONNECT = false;
...
// By default the setting is ‘US’ which disables channels 12 and 13.
// There are three possible configurations (US / JP / EU).
// Both EU and JP supports channels 12 and 13.
const char* COUNTRY = "EU";
...
bool WiFiClass::hasCredentials() {
    ...
    for (int i = 0; i < 7; ++i) {
        int ret = sl_WlanProfileGet(i, name, &nameLen, mac, &secParams, &entParams, &priority);
